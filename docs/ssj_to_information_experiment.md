# Переход к локальной HANK/SSJ-постановке
...
Текущий `jacobian_summary.csv` из HANK-ядра содержит отклики на `monetary_policy_shock`. Это
полезно для локальной проекции альтернативной траектории ставки и используется как один из
HANK/SSJ-артефактов текущей постановки.

Текущий экспорт в `outputs/ssj/jacobians.npz` следует трактовать как локальный SSJ-артефакт для
оценки правил ставки, а не как решение полной задачи оптимальной политики. Он содержит матрицы
откликов на шок денежно-кредитной политики для агрегированных переменных; дополнительные
эксперименты отдельно проверяют шок доходного риска и распределительный канал.
