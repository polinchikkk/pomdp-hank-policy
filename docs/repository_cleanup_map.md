...
В ней сохранено состояние проекта до удаления старых материалов шестого этапа: старые таблицы, графики, проверки с методом PPO, проверки переноса, HANK-проекции и дополнительные сценарии.
...
Папка `outputs/` больше не отслеживается в основной ветке. Новые расчеты создаются локально и при необходимости могут быть отдельно отобраны для текста.
...
- старые выходы этапов 2--5 из папки `outputs/`;
- старые скрипты шестого этапа `scripts/run_hank_regime_*`, кроме базового `scripts/run_hank_regime.py`;
- старые модули `hank_regime_learning_baseline` для архитектурных абляций, карты ошибок спецификации, расширений правил, проверок с методом PPO, сводки шестого этапа и проверки редуцированного состояния.
