...
- `outputs/hank_core/audit/report_hank_core_audit.md`;
...
Аудит проверяет, что HANK-ядро не является чёрным ящиком: стационарное распределение имеет массу
единица и неотрицательно, borrowing-constraint shares явно зафиксированы, policy/budget residuals и
Euler proxy residuals малы, рынки очищаются в steady state, а nonlinear transition solver сходится
по target residuals для `monetary_policy_shock`, `rstar`, `Z` и `G`. Отдельно сохраняется
диагностический `goods_mkt` residual по каждому transition shock type.
...
Для финального текста отдельные CSV сворачиваются в dashboard `гипотеза → тест → результат`.
Он включает фактические, перемешанные, искусственные, запаздывающие и остаточные признаки, а также
negative/positive controls. Логика соответствует HANK-мотивации: распределительная информация
должна помогать не из-за большего числа входов правила, а потому что MPC, ликвидность и
interest-rate exposure несут информацию о будущей трансмиссии ставки.
...
## 13. Large-sample split и основная статистика
...
Этот блок является основной статистикой статьи, а не appendix robustness. Малые прогоны
используются только как smoke-тесты.
...
- `outputs/ssj/stochastic/large_sample/hierarchical_inference.csv`;
- `outputs/ssj/stochastic/large_sample/main_inference.csv`;
...
Финальный split по умолчанию:

- train shock seeds: `0:199`;
- validation shock seeds: `200:399`;
- test shock seeds: `400:899`;
- validation observation seeds: `930:959`;
- test observation seeds: `960:999`.
...
Она пишет в `outputs/ssj/stochastic/large_sample_smoke/` и не используется как финальная таблица.

Статистическая проверка теперь использует несколько способов построения вывода:

- основной иерархический bootstrap: внешний кластер `shock_seed`, вложенный шум `observation_seed`;
...
Итоговая статья должна брать главные числа из `hierarchical_inference.csv` / `main_inference.csv`,
а не из короткого протокола на 300 траекториях.

## 14. LQG / Riccati-ориентир

Цель: построить обязательный верхний ориентир для совместной линейной state-space задачи и проверить,
насколько простые правила далеки от оптимального линейного регулятора.
...
В финальном протоколе этот блок является обязательным benchmark, а не отдельной robustness-таблицей.
Если простое правило даёт положительный MVOI, но LQG-ценность распределительных наблюдений близка к
нулю, это трактуется как проблема класса правила или оптимизации. Если LQG-ценность положительна, а
простое правило нестабильно, информационная ценность сохраняется, но проблема лежит в классе правила.
Если оба эффекта положительны, это самый сильный случай.
...
- `outputs/ssj/stochastic/final_voi_protocol/open_loop_vs_closed_loop_main_pair.csv`;
- `outputs/ssj/stochastic/final_voi_protocol/table_final_open_closed_loop_side_by_side.tex`;
- `outputs/ssj/stochastic/final_voi_protocol/table_lqg_oracle_benchmark.csv`;
- `outputs/ssj/stochastic/final_voi_protocol/lqg_oracle_diagnostics.csv`;
- `outputs/ssj/stochastic/final_voi_protocol/final_voi_protocol_gate.json`.

Финальная логика: large-sample open-loop/fixed-path результат остаётся первичной
оценкой, но closed-loop local projection является главным credibility check. LQG/Riccati oracle
является обязательным методологическим ориентиром. Финальная таблица ставит рядом
A. open-loop / fixed-path evaluation и B. closed-loop local projection. Для главной
пары `filtered_distribution_minus_filtered_aggregates` она фиксирует mean delta, cluster CI,
sign-flip p-value, win rate, convergence failure rate, max spectral radius local loop и stability
penalty. Если closed-loop знак положителен, но CI включает ноль, вывод формулируется как
`effect direction survives, precision weaker`; если знак меняется, финальный gate падает и
результат нужно диагностировать через closed-loop среду, а не спасать другими робастностями.
Отдельная LQG-таблица содержит пять строк: простое правило на фильтрованных агрегатах, простое
правило на распределительной информации, LQG на агрегатных наблюдениях, LQG на агрегатных плюс
распределительных наблюдениях и LQR полной информации.
...
Текущий финальный режим больше не допускает Toeplitz fallback как молчаливую замену:
`exp30_closed_loop_distributional_ssj.py` по умолчанию работает в strict mode, а отдельный аудит
распределительных якобианов проверяет сходимость shifted transition solves и eps-grid локальную
линейность.

## 18. Финальный механизм через локально SSJ-оптимальную ставку

Цель: доказать, что распределительные признаки полезны не только по таблице потерь, а потому что
помогают восстановить состояние трансмиссии, которое центральный банк должен учитывать при выборе
ставки. Этот блок является отдельным финальным результатом, а не вспомогательным графиком.
...
Основной финальный dashboard:

- `outputs/final_protocol/mechanism_dashboard.csv`;
- `outputs/final_protocol/mechanism_coefficients.csv`;
- `outputs/final_protocol/mechanism_coefficient_stability.csv`;
- `outputs/final_protocol/mechanism_stability_by_seed.csv`;
- `outputs/final_protocol/mechanism_bins.csv`;
- `outputs/final_protocol/report_mechanism_dashboard.md`.

Dashboard использует две целевые переменные:

- `local_optimal_rate_t`;
- `future_marginal_transmission_strength_t`.

Сравниваются четыре класса моделей:

- A. filtered aggregates only;
- B. filtered aggregates + each distributional statistic;
- C. filtered aggregates + all distributional statistics;
- D. residualized distributional statistics.

В финальном dashboard cross-fitting делается по `shock_seed`, чтобы строки, периоды и observation
noise из одной HANK/SSJ-траектории не попадали одновременно в train и test folds. На выходе
фиксируются out-of-fold R2 gain, MAE gain, coefficient signs, устойчивость выигрыша по `shock_seed`
и bins по силе распределительного сигнала.

Exploratory артефакты старого механизма:
...
- для `local_optimal_rate_t` полный распределительный набор повышает OOF R2 примерно на `0.038`
  и снижает MAE примерно на `1.63e-05` относительно filtered aggregates;
- отдельные распределительные статистики тоже дают положительный MAE gain для локально
  оптимальной ставки;
- residualized distributional statistics дают тот же incremental linear signal, что и полный
  распределительный блок после учёта агрегатов;
- для `future_marginal_transmission_strength_t` текущая state-dependent proxy не показывает
  положительный incremental gain на этих артефактах; это нужно писать как границу механизма.

Интерпретация: механизм поддержан для восстановления локально оптимальной ставки. Канал через
будущую силу трансмиссии в текущей proxy-спецификации слабее и не должен продаваться как главный
доказанный механизм.
