...
def _write_report(
    ...
    lines.extend([
        ...
        "",
        f"- Наиболее сильный отклик потребления среди robustness-сценариев даёт `{most_sensitive_robustness['scenario_label']}`: `{most_sensitive_robustness['peak_consumption_response']:.4f}`.",
    ...
    (output_dir / "report_stage2_hank_policy.md").write_text("\n".join(lines))
...
def run_pipeline(config: HANKCalibration | None = None, output_dir: str | None = None):
    ...
    _save_json(root / "model_spec.json", {
        ...
        "stage": "Этап 2. Классическая денежно-кредитная политика в полной HANK",
        "solution_method": "steady state + sequence-space Jacobian + nonlinear transition",
        "household_block": "two assets, idiosyncratic labor income, rebalancing costs",
