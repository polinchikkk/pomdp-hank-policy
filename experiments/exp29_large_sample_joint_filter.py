...
from policy.inference import bh_adjust_pvalues, summarize_paired_inference
...
def main() -> None:
    ...
    _write_report(summary_all, pairwise_all, clustered, output_dir / "report_large_sample.md")
    ...
    spec = LargeSampleSpec(
        ...
        note=(
            ...
            "Test shock paths evaluate rules. Clustered inference resamples shock_seed clusters and treats "
            "observation_seed as within-shock subclusters."
        ),
    )
    ...
    print(f"Wrote {output_dir / 'clustered_inference.csv'}")
...
def _write_report(summary: pd.DataFrame, pairwise: pd.DataFrame, clustered: pd.DataFrame, path: Path) -> None:
    ...
    lines = [
        "# Проверка на увеличенной выборке",
        "",
        "В этом прогоне отдельно задаются начальные значения для HANK/SSJ-шоков, шума наблюдений и подбора правила.",
        "",
        "## Основные потери",
        "",
    ]
