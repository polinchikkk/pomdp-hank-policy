...
> можно ли построить компактное информационное состояние, которое восстанавливает скрытую силу
> трансмиссии ставки, и в каких режимах распределительные данные нужны сверх хорошо
> отфильтрованных агрегатов?
...
→ восстановление скрытого состояния трансмиссии
...
состояния. Механистический объект -- локальный индекс силы трансмиссии ставки
\[
\tau_t(H)=
\sum_{h=1}^H
\left(
...
\right),
\]
построенный из SSJ-откликов. Распределительные статистики трактуются как сигналы о том,
насколько ставка сейчас влияет на потребление, выпуск и инфляцию, а не как механическое
расширение числа признаков.
...
стационарного состояния и проверяет, восстанавливают ли они скрытое состояние трансмиссии. Эта
логика связывает HANK-каналы неоднородности домохозяйств с постановкой неполной наблюдаемости:
регулятор не должен оцениваться так, будто он знает истинные ex post состояния.
...
Отдельная проверка компактного индекса скрытой силы трансмиссии:

```bash
python3 experiments/exp40_transmission_state_value.py
```

Скрипт строит \(\tau_t(H)\) из локальных SSJ-откликов выпуска, потребления и инфляции на ставку и
проверяет, предсказывают ли распределительные признаки этот индекс сверх фильтрованных агрегатов.
Модель F использует fold-generated compact transmission index, сжатый из всех распределительных
статистик, а не oracle-копию цели. Основные выходы:
...
Текущий полный прогон на available large-sample test artifacts показывает важную границу:
чистая SSJ-норма \(\tau_t(H)\) имеет 60 уникальных значений и в основном отражает локальный
Jacobian/horizon profile; распределительные признаки не улучшают её прогноз сверх агрегатов в
large-sample evaluation. Поэтому сильный mechanism-result сейчас следует формулировать через
восстановление локально оптимальной ставки, а не продавать чистый \(\tau_t(H)\) как уже
доказанный распределительный state variable.

Карта режимов ценности распределительной информации:

```bash
python3 experiments/exp40_distributional_value_phase_diagram.py
```

Этот блок переводит robustness-логику в содержательный policy diagram: когда центральному банку
достаточно хорошо фильтровать агрегаты, а когда распределительные данные имеют самостоятельную
ценность. Скрипт строит три карты: сила распределительного канала против шума агрегатных
наблюдений, шум агрегатных против шума распределительных наблюдений, а также persistence
распределительного состояния против веса разрыва выпуска в функции потерь. Цвет -- MVOI
`filtered_distribution` относительно `filtered_aggregates`; контуры показывают статистическую
значимость и устойчивость знака. Выходы сохраняются в
`outputs/final_protocol/phase_diagrams/`: общий
`distributional_value_phase_diagram.csv`, отдельные CSV по картам, PNG/PDF-рисунки и markdown-отчёт.
Быстрая проверка:
